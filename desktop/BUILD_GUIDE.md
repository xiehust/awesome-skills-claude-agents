# Claude Agent Platform Desktop 构建指南

本指南详细说明如何构建 Claude Agent Platform 的 Mac/Linux 桌面应用版本。
...
├── dmg/
│   └── Claude Agent Platform_1.0.0_aarch64.dmg  # DMG 安装包
└── macos/
    └── Claude Agent Platform.app/               # 应用程序包
```
...
desktop/src-tauri/target/release/bundle/
├── deb/
│   └── claude-agent-platform_1.0.0_amd64.deb   # Debian 包
└── appimage/
    └── claude-agent-platform_1.0.0_amd64.AppImage  # AppImage
```
...
# - macOS: ~/Library/Application Support/ClaudeAgentPlatform/data.db
# - Linux: ~/.local/share/claude-agent-platform/data.db
...
```json
{
  "productName": "Claude Agent Platform",
  "version": "1.0.0",
  "identifier": "com.claude-agent-platform.desktop",
  "build": {
...
| 平台 | 数据目录 |
|------|----------|
| macOS | `~/Library/Application Support/ClaudeAgentPlatform/` |
| Linux | `~/.local/share/claude-agent-platform/` |
...
或在终端运行：
```bash
xattr -cr "/Applications/Claude Agent Platform.app"
```
...
如遇到问题，请：
1. 查看日志文件 (`~/Library/Logs/ClaudeAgentPlatform/` 或终端输出)
2. 检查 [常见问题](#常见问题) 部分
